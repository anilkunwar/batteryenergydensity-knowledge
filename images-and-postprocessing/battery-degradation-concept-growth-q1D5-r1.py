...
# ═══════════════════════════════════════════════════════════════
#  DATA — Q1D5: Mechanisms of SEI growth and transition metal dissolution
#  Concepts with 0 counts removed. NMC811 shows highest growth.
# ═══════════════════════════════════════════════════════════════
data_raw = {
    "Material":  ["nmc811", "fec", "lfp", "graphite", "vc"],
    "Time_1":    [1,       18,   55,   467,       36],
    "Time_2":    [22,      49,   114,  405,       20],
    "Symbol":    ["▲",     "●",   "◆",   "■",       "▼"],
...
# Q1D5-themed palette
DEFAULT_PALETTE = {
    "nmc811":   "#D62828",   # Vivid red   — highest growth
    "fec":      "#2A9D8F",   # Teal-green  — strong additive interest
    "lfp":      "#457B9D",   # Steel blue  — stable baseline
    "graphite": "#6C757D",   # Grey        — anode material, high base
    "vc":       "#F4A261",   # Orange      — decline in interest
}
MARKER_STYLE = {
    "nmc811":   "^",   # triangle up
    "fec":      "o",   # circle
...
def plot_slope_chart(df_active, **kw):
    ...
    # --- text ---
    title    = kw.get("title_text",     "Q1D5 — SEI Growth & TM Dissolution")
    subtitle = kw.get("subtitle_text",  "Mechanisms driving capacity fade · 5 concepts compared")
...
#  STREAMLIT PAGE
# ═══════════════════════════════════════════════════════════════
st.set_page_config(page_title="Q1D5 — SEI Growth & Capacity Fade", layout="wide")
...
background:linear-gradient(90deg,#D62828,#2A9D8F,#457B9D,#6C757D);
-webkit-background-clip:text;-webkit-text-fill-color:transparent">
Q1D5 — SEI Growth & Transition Metal Dissolution</span></div>
<p style="color:#888;margin-top:-4px;margin-bottom:16px">
Concepts: nmc811, fec, lfp, graphite, vc &middot;
...
with st.sidebar:
    ...
    st.caption("ℹ️  5 concepts compared.\n"
               "NMC811 shows explosive growth (+2100%).\n"
               "VC and graphite show decline.")
    ...
    with st.expander("📌  Annotation Callout", expanded=False):
        ...
        # Default to nmc811 (index 1 in list)
    ...
    with st.expander("✨  Glow / Highlight", expanded=False):
        hi_star    = st.checkbox("Highlight nmc811 (Highest Growth)", True)
    ...
    with st.expander("📝  Titles & Text", expanded=False):
        title_t = st.text_input(
            "Title",
            "Q1D5 — SEI Growth & Transition Metal Dissolution")
        sub_t   = st.text_input("Subtitle",
                                "Mechanisms driving capacity fade · "
                                "5 concepts compared")
...
# ─── data table ──────────────────────────────────────────────
with st.expander("📊  View Raw Data  (5 concepts)", expanded=False):
    st.dataframe(
    ...
    st.caption("Concepts: nmc811, fec, lfp, graphite, vc.  "
               "NMC811 shows the highest growth rate (+2100.00%).")
    ...
    for col, fmt, ext, mime in [
        ...
        with col:
            st.download_button(
                ...
                file_name=f"Q1D5_SEI_TM_dissolution_slope_chart.{ext}",
                mime=mime, use_container_width=True)

# ─── footer ──────────────────────────────────────────────────
st.markdown("---")
st.caption(
    f"Q1D5: What are the mechanisms of SEI growth and transition metal dissolution driving capacity fade?  ·  "
