...
# ------------------- DATA -------------------
# Weights calibrated for Q1AM2: Anode Materials is highest, followed by Degradation 
# and Performance Metrics (relevant to cycle life).
data_exact = {
    "Category": ["Cathode Materials", "Anode Materials", "Electrolyte Systems",
                 "Manufacturing", "Degradation", "Performance Metrics"],
    "raw_k": [2.0, 9.0, 1.4, 1.0, 3.6, 3.0],
    "w_k": [0.10, 0.45, 0.07, 0.05, 0.18, 0.15]
}
...
def get_download_link(fig, dpi, fmt, transparent):
    ...
    href = (f'<a href="data:{MIME_TYPES[fmt]};base64,{b64}" '
            f'download="dual_axis_chart.{fmt}">Download {fmt.upper()}</a>')
    return href

# ------------------- STREAMLIT UI -------------------
st.set_page_config(page_title="Q1AM2: Silicon Anode Volume Expansion", layout="wide")
st.title("📊 Q1AM2: Overcoming Silicon Anode Volume Expansion")
st.markdown("**Query:** Overcoming silicon anode volume expansion to maintain cycle life  "
            "**Highest Domain:** Anode Materials")
