...
# ═══════════════════════════════════════════════════════════════
#  DATA — Q1CM4: Calendering Pressure & Slot Coating Effects
#  Concepts with 0 counts removed. FEC shows highest growth rate.
# ═══════════════════════════════════════════════════════════════
data_raw = {
    "Material":  ["pressure", "fec", "calendering"],
    "Time_1":    [54,          18,   8],
    "Time_2":    [118,         49,   6],
    "Symbol":    ["▲",         "●",  "◆"],
...
# Q1CM4-themed palette
DEFAULT_PALETTE = {
    "pressure":    "#E63946",   # Vivid red   — primary factor
    "fec":         "#2A9D8F",   # Teal-green  — highest growth
    "calendering": "#457B9D",   # Steel blue  — negative growth
}
MARKER_STYLE = {
    "pressure":    "o",   # circle
    "fec":         "s",   # square
    "calendering": "D",   # diamond
}
...
def plot_slope_chart(df_active, **kw):
    ...
    # --- text ---
    title    = kw.get("title_text",     "Q1CM4 — Calendering & Slot Coating Effects")
    subtitle = kw.get("subtitle_text",  "Impact on electrode densification · 3 concepts compared")
...
#  STREAMLIT PAGE
# ═══════════════════════════════════════════════════════════════
st.set_page_config(page_title="Q1CM4 — Calendering & Densification", layout="wide")
...
Q1CM4 — Calendering & Slot Coating on Densification</span></div>
<p style="color:#888;margin-top:-4px;margin-bottom:16px">
Concepts: pressure, fec, calendering &middot;
...
with st.sidebar:
    ...
    st.caption("ℹ️  3 concepts compared.\n"
               "Pressure and FEC show significant growth.\n"
               "Calendering shows a decline.")
    ...
    with st.expander("📌  Annotation Callout", expanded=False):
        ...
        ann_mat = st.selectbox(
            ...
            index=2)  # default: fec (highest growth is key insight)
    ...
    with st.expander("📝  Titles & Text", expanded=False):
        title_t = st.text_input(
            "Title",
            "Q1CM4 — Calendering & Slot Coating Effects")
        sub_t   = st.text_input("Subtitle",
                                "Impact on electrode densification · "
                                "3 concepts compared")
...
with st.expander("📊  View Raw Data  (3 concepts)", expanded=False):
    ...
    st.caption("Concepts: pressure, fec, calendering.  "
    ...
    for col, fmt, ext, mime in [
        ...
        with col:
            st.download_button(
                ...
                file_name=f"Q1CM4_densification_slope_chart.{ext}",
                mime=mime, use_container_width=True)

# ─── footer ──────────────────────────────────────────────────
st.markdown("---")
st.caption(
    f"Q1CM4: What are the effects of calendering pressure and slot coating on electrode densification?  ·  "
