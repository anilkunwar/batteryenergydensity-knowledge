...
def generate_web_legend(df_active, custom_colors, mk_over, ln_styles, theme="light", web_font_size=14):
    ...
    # Map matplotlib markers to Unicode symbols for web display
    MK_TO_UNICODE = {
        "o": "●", "s": "■", "D": "◆", "^": "▲", "v": "▼",
        ">": "▶", "<": "◀", "*": "✦", "p": "⬟", "P": "✦",
        "h": "⬡", "H": "⬢", "X": "✕", "x": "✕", "8": "✴",
    ...
    for _, row in df_active.iterrows():
        ...
        # Use the same marker mapping as matplotlib
        ...
        items_html += (
            ...
            f'<span style="position:relative;z-index:1;color:{color};font-size:{web_font_size + 4}px;'
            f'background:{bg};padding:0 5px;line-height:1;display:flex;'
            f'align-items:center;justify-content:center;">{marker}</span>'
            '</div>'
            f'<span style="font-weight:500;white-space:nowrap;overflow:hidden;'
            f'text-overflow:ellipsis;min-width:0;font-size:{web_font_size}px;">' + mat + '</span>'
            '</div>'
        )
    ...
    html = (
        ...
        f'font-family:sans-serif;font-size:{web_font_size}px;color:{text};'
...
def plot_slope_chart(df_active, **kw):
    ...
    ax.set_xticks([1, 2])
    ax.set_xticklabels([xtick_left, xtick_right],
                       fontsize=fs + 2, fontweight="bold", color=txt_c)
    ax.set_xlabel(xl_text, fontsize=fs + 2, fontweight="bold", color=txt_c, labelpad=xlabel_pad)
    ax.set_ylabel(yl_text, fontsize=fs + 2, fontweight="bold", color=txt_c, labelpad=ylabel_pad)
...
with st.sidebar:
    ...
    with st.expander("⚙️  Axes & Grid", expanded=False):
        ...
        st.markdown("**X-Axis Tick Labels**")
        c_lbl1, c_lbl2 = st.columns(2)
        with c_lbl1:
            xtick_left = st.text_input(
                "Left Label (e.g., Early Period)", 
                "Early Period", 
                help="Enter custom text or leave empty to hide the left label"
            )
        with c_lbl2:
            xtick_right = st.text_input(
                "Right Label (e.g., Recent Period)", 
                "Recent Period", 
                help="Enter custom text or leave empty to hide the right label"
            )
        ...
        if web_legend:
            web_font_size = st.slider("Web Legend Font Size", 8, 32, 14, 1)
        else:
            web_font_size = 14
        ...
        if show_legend:
            ...
            leg_font_size = st.slider("Matplotlib Legend Font Size", 6, 24, 10, 1)
    ...
    with c_fig1:
        fw_val = st.number_input("Figure Width (inches)", min_value=2.0, max_value=30.0, value=10.0, step=0.5)
    with c_fig2:
        fh_val = st.number_input("Figure Height (inches)", min_value=2.0, max_value=30.0, value=6.5, step=0.5)
...
fig = plot_slope_chart(
    ...
    show_grid=show_grid,          grid_style=grid_sty,
    ...
    xlabel_pad=xlabel_pad,        ylabel_pad=ylabel_pad,
    tick_pad=tick_pad
)
