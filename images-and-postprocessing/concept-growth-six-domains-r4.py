...
# ═══════════════════════════════════════════════════════════════
def generate_web_legend(df_active, custom_colors, mk_over, ln_styles, theme="light", web_leg_font_size=14):
    if len(df_active) == 0:
        return ""
    ...
    html = (
        ...
        f'font-family:sans-serif;font-size:{web_leg_font_size}px;color:{text};'
...
def plot_slope_chart(df_active, **kw):
    ...
    ann_offset_y  = kw.get("ann_offset_y",       80)
    ...
    axes_label_fs = kw.get("axes_label_fs", 14)
    xl_pad        = kw.get("xlabel_pad", 10)
    yl_pad        = kw.get("ylabel_pad", 10)
    title_pad     = kw.get("title_pad", 15)
    ...
    ax.set_xticklabels(["Early Period", "Recent Period"],
                       fontsize=axes_label_fs, fontweight="bold", color=txt_c)
    
    if xl_text:
        ax.set_xlabel(xl_text, fontsize=axes_label_fs, color=txt_c, labelpad=xl_pad)
    if yl_text:
        ax.set_ylabel(yl_text, fontsize=axes_label_fs, color=txt_c, labelpad=yl_pad)

    full_title = title + (f"\n{subtitle}" if subtitle else "")
    ax.set_title(full_title, fontsize=fs + 5, fontweight="bold",
                 color=txt_c, pad=title_pad, linespacing=1.4)
    ...
    ax.tick_params(axis="both", labelsize=fs, colors=txt_c,
                   length=tk_len, width=tk_w, pad=tick_pad)
    ax.set_xlim(0.5, 2.5)
...
with st.sidebar:
    ...
    with st.expander("📌  Annotation Callout", expanded=False):
        ...
        if ann_rowkey:
            ...
            ann_curve_rad = st.slider("Arrow Curve",
                                      -0.5, 0.5, -0.2, 0.05)
            ann_offset    = st.slider("Callout Y-Offset",
                                      0, 500, 80, 10)
    ...
    with st.expander("📝  Titles & Text", expanded=False):
        ...
        st.markdown("**Axes & Title Padding**")
        c1, c2 = st.columns(2)
        with c1:
            axes_label_fs = st.slider("Axes Label Font Size", 8, 30, 14, 1)
            xl_pad = st.slider("X-Axis Label Padding", 0, 50, 10, 1)
        with c2:
            title_pad = st.slider("Title Padding", 0, 50, 15, 1)
            yl_pad = st.slider("Y-Axis Label Padding", 0, 50, 10, 1)
    ...
    with st.expander("⚙️  Axes & Grid", expanded=False):
        ...
        if show_legend:
            ...
            leg_font_size = st.slider("Matplotlib Legend Font Size", 6, 24, 10, 1, key="mfs")
        ...
        tick_pad = st.slider("Tick Label Padding", 0, 20, 5, 1)

    st.divider()
    st.subheader("🎨  Theme & Layout")
    bg_st  = st.radio("Theme", ["Light", "Dark"], horizontal=True)
    mk_sz  = st.slider("Marker Size", 4, 28, 10)
    fs_val = st.slider("Base Font Size",   8, 26, 12)
    
    st.markdown("**Figure Size (inches)**")
    c1, c2 = st.columns(2)
    with c1:
        fw_val = st.slider("Width", 4.0, 24.0, 10.0, 0.5, key="fw")
    with c2:
        fh_val = st.slider("Height", 3.0, 18.0, 6.5, 0.5, key="fh")
...
fig = plot_slope_chart(
    ...
    ann_arrow_lw=ann_arrow_lw,    ann_offset_y=ann_offset,
    ...
    axes_label_fs=axes_label_fs,  xlabel_pad=xl_pad,
    ylabel_pad=yl_pad,            title_pad=title_pad,
    tick_pad=tick_pad,
)
...
if web_legend and fig is not None:
    web_legend_html = generate_web_legend(
        ...
        theme=bg_st.lower(), web_leg_font_size=web_leg_font_size
    )
    if web_legend_html:
        try:
