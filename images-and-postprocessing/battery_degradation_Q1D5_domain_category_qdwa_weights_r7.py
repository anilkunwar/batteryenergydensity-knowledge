...
# ------------------- DATA -------------------
# QDWA scoring for Q1D5 — Bayesian-smoothed weights:
#
#   W_k = (α + raw_k) / (6·α + Σ_j raw_j),   α = 0.25
#
# Weights calibrated for Q1D5: Degradation is highest by a wide margin —
# the query focuses on mechanisms of capacity fade (SEI growth, TM dissolution).
# Anode Materials (SEI site) and Electrolyte Systems (SEI composition/dissolution medium)
# follow closely. Cathode Materials (TM dissolution source) is present but secondary
# to the degradation mechanism itself. Performance Metrics (capacity fade outcome)
# and Manufacturing receive minor weights.
#
...
QDWA_TABLE = [
    ...
    ("Degradation",         5.0,           1.5,           0.0),   # raw_k = 6.5
    ("Anode Materials",     3.5,           1.5,           0.0),   # raw_k = 5.0
    ("Electrolyte Systems", 3.0,           1.0,           0.0),   # raw_k = 4.0
    ("Cathode Materials",   2.8,           0.7242,        0.0),   # raw_k = 3.5242 (from prompt)
    ("Performance Metrics", 1.2,           0.3,           0.0),   # raw_k = 1.5
    ("Manufacturing",       0.3,           0.2,           0.0),   # raw_k = 0.5
]
...
def plot_dual_axis(df, cfg):
    ...
    ax1.set_title(f"Q1D5 Dual-Axis Chart — {version} Data",
...
def get_download_link(fig, dpi, fmt, transparent):
    ...
    href = (f'<a href="data:{MIME_TYPES[fmt]};base64,{b64}" '
            f'download="Q1D5_dual_axis_chart.{fmt}">Download {fmt.upper()}</a>')
    return href

# ------------------- STREAMLIT UI -------------------
st.set_page_config(page_title="Q1D5: SEI Growth & TM Dissolution", layout="wide")
st.title("📊 Q1D5: Mechanisms of SEI Growth and Transition Metal Dissolution Driving Capacity Fade")
st.markdown("**Query:** What are the mechanisms of SEI growth and transition metal dissolution "
            "driving capacity fade?  "
            "**Highest Domain:** Degradation")
