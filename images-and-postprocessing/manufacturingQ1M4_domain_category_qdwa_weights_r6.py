...
# ------------------- DATA -------------------
# QDWA scoring for Q1M4 — Bayesian-smoothed weights:
#
#   W_k = (α + raw_k) / (6·α + Σ_j raw_j),   α = 0.25
#
# where  raw_k = Σ_kw 1[kw ∈ k] + Σ_c w_c·1[cat(c) = k] + Σ_{t∈q} m_k(t)
#        (keyword hits)   (concept mass)        (soft-term sum)
#
# Weights calibrated for Q1M4: Manufacturing is highest by a wide margin —
# the query names two fabrication processes (calendering pressure, slot-die
# coating) and the direct outcome (electrode densification). Cathode & Anode
# Materials follow (electrode = active layer), then Performance Metrics
# (density / porosity / adhesion). Degradation and Electrolyte Systems
# receive minor secondary weight.
#
...
QDWA_TABLE = [
    ...
    ("Manufacturing",       6.5,           1.8,           0.2),   # raw_k = 8.5
    ("Cathode Materials",   2.8,           1.2,           0.2),   # raw_k = 4.2
    ("Anode Materials",     2.5,           1.1,           0.2),   # raw_k = 3.8
    ("Performance Metrics", 2.0,           1.3,           0.2),   # raw_k = 3.5
    ("Degradation",         1.2,           0.6,           0.2),   # raw_k = 2.0
    ("Electrolyte Systems", 0.8,           0.5,           0.2),   # raw_k = 1.5
]
...
def plot_dual_axis(df, cfg):
    ...
    ax1.set_title(f"Q1M4 Dual-Axis Chart — {version} Data",
...
def get_download_link(fig, dpi, fmt, transparent):
    ...
    href = (f'<a href="data:{MIME_TYPES[fmt]};base64,{b64}" '
            f'download="Q1M4_dual_axis_chart.{fmt}">Download {fmt.upper()}</a>')
    return href

# ------------------- STREAMLIT UI -------------------
st.set_page_config(page_title="Q1M4: Calendering & Slot-Coating Densification", layout="wide")
st.title("📊 Q1M4: Effects of Calendering Pressure & Slot Coating on Electrode Densification")
st.markdown("**Query:** What are the effects of calendering pressure and slot coating "
            "on electrode densification?  "
            "**Highest Domain:** Manufacturing")
