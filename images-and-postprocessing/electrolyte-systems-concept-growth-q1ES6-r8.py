...
# ═══════════════════════════════════════════════════════════════
#  DATA — Q1ES6: Optimize FEC additives and solid electrolytes
#  for high rate capability.
#  7 zero-count concepts excluded (ionic_conductivity,
#  solid_electrolyte, rate_capability, c_rate, dendrite_growth,
#  impedance_growth, liquid_electrolyte). FEC shows highest growth.
# ═══════════════════════════════════════════════════════════════
data_raw = {
    "Material":  ["fec", "graphite", "porosity"],
    "Time_1":    [18,    467,        175],
    "Time_2":    [49,    405,        125],
    "Symbol":    ["●",   "■",        "◆"],
...
# Q1ES6-themed palette
DEFAULT_PALETTE = {
    "fec":      "#D62828",   # Vivid red   — highest growth, query focus
    "graphite": "#6C757D",   # Grey        — high base, slow decline
    "porosity": "#457B9D",   # Steel blue  — declining interest
}
MARKER_STYLE = {
    "fec":      "o",   # circle
    "graphite": "s",   # square
...
def plot_slope_chart(df_active, **kw):
    ...
    # --- text ---
    title    = kw.get("title_text",     "Q1ES6 — FEC Additives & Solid Electrolytes")
    subtitle = kw.get("subtitle_text",  "Optimization for high rate capability · 3 concepts compared")
...
#  STREAMLIT PAGE
# ═══════════════════════════════════════════════════════════════
st.set_page_config(page_title="Q1ES6 — FEC Additives & High Rate Capability", layout="wide")
...
Q1ES6 — Optimize FEC Additives & Solid Electrolytes for High Rate Capability</span></div>
<p style="color:#888;margin-top:-4px;margin-bottom:16px">
Concepts: fec, graphite, porosity &middot;
Early vs Recent Publication Counts</p>""")
...
with st.sidebar:
    ...
    st.caption("ℹ️  3 concepts compared.\n"
               "FEC additive research surging (+172.22%).\n"
               "Graphite and porosity declining.\n"
               "7 zero-count concepts excluded.")
    ...
    with st.expander("📌  Annotation Callout", expanded=False):
        ...
        # Default to fec (index 1 in list — highest growth)
    ...
    with st.expander("✨  Glow / Highlight", expanded=False):
        hi_star    = st.checkbox("Highlight fec (Highest Growth)", True)
    ...
    with st.expander("📝  Titles & Text", expanded=False):
        title_t = st.text_input(
            "Title",
            "Q1ES6 — FEC Additives & Solid Electrolytes")
        sub_t   = st.text_input("Subtitle",
                                "Optimization for high rate capability · "
                                "3 concepts compared")
...
# ─── data table ──────────────────────────────────────────────
with st.expander("📊  View Raw Data  (3 concepts)", expanded=False):
    st.dataframe(
    ...
    st.caption("Concepts: fec, graphite, porosity.  "
               "FEC shows the highest growth rate (+172.22%).  "
               "7 zero-count concepts excluded "
               "(ionic_conductivity, solid_electrolyte, rate_capability, "
               "c_rate, dendrite_growth, impedance_growth, "
               "liquid_electrolyte).")
    ...
    for col, fmt, ext, mime in [
        ...
        with col:
            st.download_button(
                ...
                file_name=f"Q1ES6_FEC_high_rate_slope_chart.{ext}",
                mime=mime, use_container_width=True)

# ─── footer ──────────────────────────────────────────────────
st.markdown("---")
st.caption(
    f"Q1ES6: How to optimize FEC additives and solid electrolytes for high rate capability?  ·  "
