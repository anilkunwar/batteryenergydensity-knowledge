...
def plot_slope_chart(df, line_width, marker_size, font_size, colormap_name,
    ...
    for idx, row in df.iterrows():
        ...
        # Annotate start and end values with growth rate
        if show_labels:
            # Start point label
    ...
    # Legend (if not showing labels on points, we need a legend)
    if not show_labels:
...
colormap_name = st.sidebar.selectbox("Colormap", ["tab10", "Set1", "Set2", "Dark2", "Paired", "viridis", "plasma", "inferno", "magma"])
