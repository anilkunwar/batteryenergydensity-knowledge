...
# ═══════════════════════════════════════════════════════════════
#  DATA — Q4PM1: Silicon-Graphite Composite Architectures
#         & Prelithiation Protocols
#  *** energy_density and other 0-0 attributes excluded ***
# ═══════════════════════════════════════════════════════════════
data_raw = {
    "Material":  ["Silicon", "Graphite", "NMC811", "Prelithiation"],
    "Time_1":    [275, 467, 1, 34],
    "Time_2":    [311, 405, 22, 23],
    "Symbol":    ["◆", "●", "■", "▲"],
...
# Q4PM1-themed palette: composite architecture & prelithiation context
DEFAULT_PALETTE = {
    "Silicon":       "#9B5DE5",   # Vibrant purple  — Si/C composite hybrid nature
    "Graphite":      "#2B2D42",   # Dark charcoal  — the conventional component
    "NMC811":        "#F15BB5",   # Hot pink/magenta — high-energy cathode pairing
    "Prelithiation": "#00BBF9",   # Bright cyan    — pre-treatment protocol
}
MARKER_STYLE = {
    "Silicon":       "D",         # Diamond  — composite value, structured blend
    "Graphite":      "o",         # Circle   — conventional, baseline
    "NMC811":        "s",         # Square   — structured layered cathode
    "Prelithiation": "^",         # Triangle — pointing toward lithium addition
}
...
def plot_slope_chart(df_active, **kw):
    ...
    title    = kw.get("title_text",
                      "Q4PM1 — Si-C Composites & Prelithiation")
    subtitle = kw.get("subtitle_text",
                      "Overcoming volumetric expansion to unlock cell-level energy density")
...
# ═══════════════════════════════════════════════════════════════
st.set_page_config(
    page_title="Q4PM1 — Si-C Composites", layout="wide")
...
background:linear-gradient(90deg,#9B5DE5,#00BBF9,#F15BB5);
-webkit-background-clip:text;-webkit-text-fill-color:transparent">
Q4PM1 — Si-C Composites & Prelithiation</span></div>
<p style="color:#888;margin-top:-4px;margin-bottom:16px">
Concept Growth Analysis &middot; Pre-2021 vs Post-2021 &middot;
0-0 Attributes Excluded &middot; Volumetric Expansion Mitigation</p>""")
...
with st.sidebar:
    ...
    st.caption("ℹ️  Concepts with 0 in both periods "
               "(e.g. energy_density) are excluded.\n"
               "<strong>Key insight:</strong> Prelithiation declining (−32%) "
               "despite being a key query term — suggesting a shift toward "
               "<strong>integrated composite solutions</strong> over "
               "standalone prelithiation protocols.",
    ...
    with st.expander("📌  Annotation Callout", expanded=False):
        ...
        ann_mat = st.selectbox(
            ...
            index=4)  # default: Prelithiation (-32% decline is key insight)
    ...
    with st.expander("✨  Glow / Highlight", expanded=False):
        hi_star    = st.checkbox("Highlight Silicon (Composite Core)", True)
    ...
    with st.expander("📝  Titles & Text", expanded=False):
        ...
        sub_t   = st.text_input("Subtitle",
                                "Overcoming volumetric expansion to unlock "
...
border-left:4px solid #9B5DE5;margin-bottom:16px">
<strong>📊 Q4PM1 Insight:</strong> The −32% decline in <strong>Prelithiation</strong>
(▲ cyan) is counterintuitive — this is a key query term. It suggests a strategic
pivot: researchers are increasingly favoring <strong style="color:#9B5DE5">
integrated Si-C composite architectures</strong> (◆ purple, +13%)
over standalone prelithiation protocols. <strong>Graphite</strong> (● gray, −13%)
declines as composites become Si-dominant rather than graphite-dominant.
<strong>NMC811</strong> (■ magenta, +2,100%) surges as the cathode companion
for high-energy composite anodes.
</div>
""", unsafe_allow_html=True)
...
with st.expander("📊  View Raw Data  (0-0 attributes excluded)", expanded=False):
    ...
    st.caption("Excluded (0 in both periods): energy_density "
               "and other zero-count concepts from the Q4PM1 attribute set.")
    ...
    for col, fmt, ext, mime in [
        ...
        with col:
            st.download_button(
                ...
                file_name=f"Q4PM1_slope_chart.{ext}",
                mime=mime, use_container_width=True)

# ─── footer ──────────────────────────────────────────────────
st.markdown("---")
st.caption(
    f"Q4PM1: Si-C Composite Architectures & Prelithiation Protocols  ·  "
