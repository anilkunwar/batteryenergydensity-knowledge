...
- 6 color themes (Bright, Dark, Midnight, Warm, Forest, Ocean)
- Physics presets (Stable, Fluid, Tight, Off) with deterministic pre-layout
...
THEMES = {
    "Bright": {
        "bg": "#ffffff", "paper": "#ffffff", "font": "#000000",
        "grid": "#e5e7eb", "axis": "#374151", "tooltip_bg": "#1f2937",
        "tooltip_font": "#f3f4f6", "edge_cooc": "#38bdf8", "edge_sem": "#fb923c",
        "edge_bridge": "#facc15", "node_border": "#ffffff", "shadow": "rgba(0,0,0,0.15)",
        "accent": "#3b82f6", "muted": "#6b7280", "pyvis_bg": "#ffffff",
        "table_header": "#f3f4f6", "table_border": "#e5e7eb"
    },
    "Dark": {
        "bg": "#0f172a", "paper": "#0f172a", "font": "#e2e8f0",
        "grid": "#1e293b", "axis": "#94a3b8", "tooltip_bg": "#1e293b",
        "tooltip_font": "#f1f5f9", "edge_cooc": "#7dd3fc", "edge_sem": "#fdba74",
        "edge_bridge": "#fde047", "node_border": "#334155", "shadow": "rgba(0,0,0,0.4)",
        "accent": "#60a5fa", "muted": "#64748b", "pyvis_bg": "#0f172a",
        "table_header": "#1e293b", "table_border": "#334155"
    },
    "Midnight": {
        "bg": "#020617", "paper": "#020617", "font": "#cbd5e1",
        "grid": "#0f172a", "axis": "#64748b", "tooltip_bg": "#0f172a",
        "tooltip_font": "#e2e8f0", "edge_cooc": "#22d3ee", "edge_sem": "#f97316",
        "edge_bridge": "#eab308", "node_border": "#1e293b", "shadow": "rgba(0,0,0,0.5)",
        "accent": "#38bdf8", "muted": "#475569", "pyvis_bg": "#020617",
        "table_header": "#0f172a", "table_border": "#1e293b"
    },
    "Warm": {
        "bg": "#fff7ed", "paper": "#fff7ed", "font": "#431407",
        "grid": "#fed7aa", "axis": "#78350f", "tooltip_bg": "#431407",
        "tooltip_font": "#fff7ed", "edge_cooc": "#0ea5e9", "edge_sem": "#f97316",
        "edge_bridge": "#eab308", "node_border": "#ffedd5", "shadow": "rgba(124,45,18,0.15)",
        "accent": "#ea580c", "muted": "#a16207", "pyvis_bg": "#fff7ed",
        "table_header": "#ffedd5", "table_border": "#fed7aa"
    },
    "Forest": {
        "bg": "#f0fdf4", "paper": "#f0fdf4", "font": "#052e16",
        "grid": "#bbf7d0", "axis": "#166534", "tooltip_bg": "#052e16",
        "tooltip_font": "#f0fdf4", "edge_cooc": "#0ea5e9", "edge_sem": "#f97316",
        "edge_bridge": "#eab308", "node_border": "#dcfce7", "shadow": "rgba(21,128,61,0.15)",
        "accent": "#16a34a", "muted": "#3f6212", "pyvis_bg": "#f0fdf4",
        "table_header": "#dcfce7", "table_border": "#bbf7d0"
    },
    "Ocean": {
        "bg": "#ecfeff", "paper": "#ecfeff", "font": "#083344",
        "grid": "#a5f3fc", "axis": "#155e75", "tooltip_bg": "#083344",
        "tooltip_font": "#ecfeff", "edge_cooc": "#0ea5e9", "edge_sem": "#f97316",
        "edge_bridge": "#eab308", "node_border": "#cffafe", "shadow": "rgba(14,116,144,0.15)",
        "accent": "#0891b2", "muted": "#0e7490", "pyvis_bg": "#ecfeff",
        "table_header": "#cffafe", "table_border": "#a5f3fc"
    }
}
...
def get_theme() -> Dict[str, str]:
    ...
    theme_name = st.session_state.get('theme', 'Bright')
    return THEMES.get(theme_name, THEMES["Bright"])

# ==========================================
# ROBUST JSON LOADER (from JSON Metadata Explorer)
# ==========================================
...
BATTERY_PATTERNS = [
    r'(?:\d+(?:\.\d+)?\s*(?:wh/kg|wh kg-1|wh kg⁻¹|wh l-1|wh l⁻¹|mah/g|mah g-1|mah g⁻¹|mah/cm³|mah cm-3))',
    r'(?:Li(?:[A-Z][a-z]?\d*)+(?:O\d*)?)',
    r'(?:NCM|NMC|LCO|LMO|LFP|LMNO|LNMO|NCA|LNO|LTO)\d*(?:\d+(?:\.\d+)?)?',
    r'(?:18650|21700|4680|26650|14500)',
    r'(?:solid.?state|all.?solid.?state)',
    r'(?:fast.?charge|quick.?charge|rapid.?charge)',
    r'(?:high.?energy|high.?power|long.?life)',
    r'(?:Si(?:Ox?)?|SiO\d*|silicon.?oxide)',
    r'(?:prelithiat(?:ed|ion))',
    r'(?:3D.?print(?:ed|ing)|additive.?manufactur(?:ed|ing))'
]
    ...
    for concept in concepts:
        ...
        for pattern, category in BATTERY_CATEGORY_MAPPING.items():
            if re.search(pattern, concept, re.I):
                ...
                break
        if not matched:
            if any(re.search(p, concept, re.I) for p in [r'Li[A-Z]', r'NCM', r'NMC', r'LFP', r'LCO']):
...
def compute_deterministic_layout(nx_graph, seed=42):
    """Pre-compute a deterministic layout to eliminate dancing."""
    n_nodes = len(nx_graph.nodes())
    if n_nodes < 3:
        return nx.spring_layout(nx_graph, seed=seed)
    try:
        if n_nodes <= 300:
            pos = nx.kamada_kawai_layout(nx_graph)
        else:
            pos = nx.spring_layout(nx_graph, k=1.5, iterations=100, seed=seed, weight='weight')
    except Exception:
        pos = nx.spring_layout(nx_graph, k=1.5, iterations=100, seed=seed, weight='weight')
    return pos
...
def render_graph_pyvis(nx_graph, concept_abstract_map, physics_enabled=True,
    ...
    """
    Stable, polished PyVis renderer with deterministic pre-layout,
    theme support, and tuned physics.
    """
    if theme is None:
        theme = THEMES["Bright"]
    ...
    pos = compute_deterministic_layout(nx_graph)

    cmap_colors = get_colormap_colors(cmap_name, max(1, len(nx_graph.nodes())))

    # PyVis configuration with theme
    net = Network(
        height="750px", width="100%",
        bgcolor=theme["pyvis_bg"], font_color=theme["font"],
    ...
    if physics_enabled and physics_preset.get("gravity", 0) != 0:
        net.barnes_hut(
            gravity=physics_preset["gravity"],
            central_gravity=physics_preset["central_gravity"],
            spring_length=physics_preset["spring_length"],
            spring_strength=physics_preset["spring_strength"],
            damping=physics_preset["damping"],
            overlap=0.15
        )
        # Force stabilization before display
        ...
        # Completely static — disable physics
        net.set_options(json.dumps({
            "physics": {"enabled": False, "stabilization": {"enabled": False}}
        }))

    # Global options for polish
    ...
    # Build nodes with fixed positions and theme-aware styling
    for i, node in enumerate(nx_graph.nodes()):
        freq = len(concept_abstract_map.get(node, []))
        degree = int(nx_graph.degree(node))

        # Size by frequency with mass scaling for stability
        size = float(np.clip(min_node_size + freq * 1.5, min_node_size, max_node_size))
        mass = 1.0 + (freq / max(1, len(concept_abstract_map))) * 4.0
        ...
        net.add_node(
            ...
            color={
                ...
                "highlight": {"background": color, "border": theme["accent"]},
                "hover": {"background": color, "border": theme["accent"]}
            },
            font={
                "color": theme["font"],
                "size": node_label_size,
                "face": "Inter, Segoe UI, Roboto, sans-serif",
                "strokeWidth": 2 if theme["bg"] == "#ffffff" or theme["bg"] == "#fff7ed" or theme["bg"] == "#f0fdf4" or theme["bg"] == "#ecfeff" else 0,
            ...
            <div style="
                ...
                <b style="font-size:14px; color:{theme['accent']}">{node}</b><br/>
                <span style="opacity:0.8">Degree:</span> <b>{degree}</b><br/>
                <span style="opacity:0.8">Frequency:</span> <b>{freq}</b><br/>
                <span style="opacity:0.8">Mass:</span> <b>{mass:.2f}</b>
            </div>
            """,
            shadow={
                "enabled": True,
                "color": theme["shadow"].replace("rgba", "").replace("(", "").replace(")", "").split(",")[:3],
                "size": 10,
                "x": 3,
                "y": 3
            }
    ...
    # Build edges with theme-aware colors and smooth curves
    edge_color_map = {
        'cooccurrence': theme["edge_cooc"],
        'semantic': theme["edge_sem"],
    ...
    for u, v in nx_graph.edges():
        ...
        color = edge_color_map.get(edge_type, theme["muted"])

        # Convert hex to rgba for transparency
        hex_color = color.lstrip('#')
        r, g, b = tuple(int(hex_color[i:i+2], 16) for i in (0, 2, 4))
        rgba_color = f"rgba({r},{g},{b},0.65)"

        net.add_edge(
            u, v,
            value=float(np.clip(w, 0.5, 5)),
            width=float(np.clip(w * 0.6, 1, 3)),
            color={"color": rgba_color, "highlight": color, "hover": color},
            smooth={"type": "continuous", "roundness": 0.3},
            title=f"<span style='color:{theme['font']}; font-family:Inter,sans-serif;'>Weight: {w:.2f}<br/>Type: {edge_type}</span>"
        )
...
def render_graph_plotly_2d(nx_graph, concept_abstract_map, cmap_name="viridis", 
    ...
    edge_trace = go.Scatter(x=edge_x, y=edge_y, mode='lines',
                            line=dict(width=1, color=theme["muted"]),
    ...
    fig = go.Figure(data=[edge_trace, node_trace],
                    layout=go.Layout(showlegend=False, hovermode='closest',
                                     ...
                                     xaxis=dict(showgrid=False, zeroline=False, showticklabels=False, color=theme["axis"]),
                                     yaxis=dict(showgrid=False, zeroline=False, showticklabels=False, color=theme["axis"])))
    st.plotly_chart(fig, use_container_width=True)

def render_graph_plotly_3d(nx_graph, concept_abstract_map, cmap_name="viridis", top_n_nodes=0, theme=None):
    if theme is None:
        theme = THEMES["Bright"]
    ...
    edge_trace = go.Scatter3d(x=edge_x, y=edge_y, z=edge_z, mode='lines',
                              line=dict(width=2, color=theme["muted"]), hoverinfo='skip')
    ...
    fig = go.Figure(data=[edge_trace, node_trace],
                    layout=go.Layout(scene=dict(xaxis=dict(showbackground=False, color=theme["axis"]),
                                                 yaxis=dict(showbackground=False, color=theme["axis"]),
                                                 zaxis=dict(showbackground=False, color=theme["axis"]),
                                                 bgcolor=theme["bg"]),
                                     margin=dict(l=0, r=0, b=0, t=0), showlegend=False,
                                     paper_bgcolor=theme["paper"], font=dict(color=theme["font"])))
    st.plotly_chart(fig, use_container_width=True)
...
def render_sunburst_chart(labels, parents, values, cmap_name="viridis", label_size=11, width=800, height=600, theme=None):
    ...
    fig = go.Figure(go.Sunburst(
        ...
        marker=dict(colors=colors, line=dict(width=0.5, color=theme["node_border"])),
        textinfo="label+percent entry+value",
        insidetextorientation="radial",
        textfont=dict(size=label_size, color=theme["font"]),
    ...
    fig.update_layout(
        ...
        font=dict(size=label_size, family="Arial", color=theme["font"]),
...
def render_radar_chart(concept_scores_df: pd.DataFrame, top_k: int = 15, cmap_name: str = "viridis", theme=None):
    ...
    fig.update_layout(
        polar=dict(radialaxis=dict(visible=True, range=[0, 1], color=theme["axis"]),
                   angularaxis=dict(color=theme["axis"]),
                   bgcolor=theme["bg"]),
        title="Top Concepts: Multi-Dimensional Comparison",
        showlegend=True, width=750, height=600,
        paper_bgcolor=theme["paper"], font=dict(color=theme["font"]),
        legend=dict(orientation="h", yanchor="bottom", y=-0.2, font=dict(color=theme["font"]))
    )
    st.plotly_chart(fig, use_container_width=True)
...
def main():
    ...
    if st.session_state.analysis_data is not None:
        ...
        top_n_graph = st.session_state.get('top_n_graph', 0)
